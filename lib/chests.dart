...
class ChestsPage extends StatelessWidget {
      ...
      child: Scaffold(
        ...
        body: Padding(
          ...
          child: StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance.collection('chests').orderBy('id').snapshots(),
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text('الرصيد: ${chest['balance']?.toString() ?? '0'}'),
                          Text('الحصة: ${chest['share']?.toString() ?? '0'}'),
                          Row(
                            ...
                            children: [
                              ...
                              TextButton(
                                onPressed: () => _editChest(context, docId, chest),
                                child: const Text(
                                  'تعديل',
  ...
  Future<void> _editChest(BuildContext context, String docId, Map<String, dynamic> chest) async {
    final nameController = TextEditingController(text: chest['name']);
    final balanceController = TextEditingController(text: chest['balance'].toString());
    final shareController = TextEditingController(text: chest['share'].toString());
        ...
        return Directionality(
          ...
          child: AlertDialog(
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () async {
                  await FirebaseFirestore.instance.collection('chests').doc(docId).update({
                    'name': nameController.text,
                    'balance': int.parse(balanceController.text),
                    'share': int.parse(shareController.text),
                    'updated_at': DateTime.now().toIso8601String(),
