...
class CaseProfile extends StatelessWidget {
  ...
  List<Widget> _buildDetails(Map<String, dynamic> caseData) {
    ...
    final details = [
      ...
      {
        'label': 'العمر:',
        'value': AgeCalculator.calculateAge(
                caseData['birth_date']?.toString(), caseData['age'])
            .toString()
      },
      {
  ...
  Future<void> _addCash(BuildContext context, int caseId) async {
    ...
    try {
      ...
      final caseData = caseSnapshot.docs.first.data();
  ...
  void _printReceipt(
    ...
    try {
      ...
      // Receipt Title
      buffer.writeln(PrintStyle.getHeader('إيصال القبض'));
