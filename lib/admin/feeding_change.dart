...
class FeedingChangePage extends StatefulWidget {
  const FeedingChangePage({Key? key}) : super(key: key);
...
class _FeedingChangePageState extends State<FeedingChangePage> {
  ...
  Future<List<String>> fetchFeedingDocIds() async {
    final snapshot = await FirebaseFirestore.instance.collection('feedings').get();
  ...
  Future<void> renameFeedingDoc(String oldId, String newId) async {
    ...
    try {
      final docRef = FirebaseFirestore.instance.collection('feedings').doc(oldId);
      final docSnap = await docRef.get();
      if (!docSnap.exists) throw Exception('المستند غير موجود');
      final data = docSnap.data();

      // Create new doc with newId
      await FirebaseFirestore.instance.collection('feedings').doc(newId).set(data!);
      // Delete old doc
      await docRef.delete();
            ...
            builder: (context, snapshot) {
              ...
              return Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 20),
                  ElevatedButton(
                    onPressed: isLoading || selectedDocId == null || newDateController.text.isEmpty
                        ? null
                        : () => renameFeedingDoc(selectedDocId!, newDateController.text.trim()),
                    child: isLoading
