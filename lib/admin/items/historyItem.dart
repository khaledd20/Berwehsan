...
class _HistoryItemPageState extends State<HistoryItemPage> {
          ...
          children: [
            ...
            Expanded(
              child: StreamBuilder<QuerySnapshot>(
                stream: FirebaseFirestore.instance.collection('store_log').snapshots(),
                      ...
                      return Card(
                        ...
                        child: ListTile(
                          ...
                          subtitle: Column(
                            ...
                            children: [
                              Text('الحالة: ${status == 'in' ? 'إضافة' : 'سحب'}',
                                  textAlign: TextAlign.right),
                              Text('العدد: $count', textAlign: TextAlign.right),
                              Text('تاريخ الإنشاء: $createdAt', textAlign: TextAlign.right),
                            ],
                          ),
                        ),
                      );
  ...
  final snapshot = await FirebaseFirestore.instance.collection('store_log').get();
  ...
  final filteredLogs = snapshot.docs.where((log) {
    ...
    if (itemIdFilter.isNotEmpty && !itemId.toString().contains(itemIdFilter)) {
      return false;
    }
  ...
  buffer.writeln('<tr><th>معرف العنصر</th><th>العدد</th><th>الحالة</th><th>تاريخ الإنشاء</th></tr>');
