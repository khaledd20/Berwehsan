...
class LinkCasesToSubPage extends StatefulWidget {
  ...
  const LinkCasesToSubPage({Key? key, required this.subId, required this.subName}) : super(key: key);
...
class _LinkCasesToSubPageState extends State<LinkCasesToSubPage> {
  ...
  Future<void> loadCases() async {
    final querySnapshot = await FirebaseFirestore.instance.collection('cases').get();
  ...
  for (var doc in allCases) {
    ...
    updateFutures.add(
      FirebaseFirestore.instance.collection('cases').doc(docId).update({'sub_ids': currentSubIds}),
    );
  }
  ...
  Widget build(BuildContext context) {
    final filteredCases = allCases.where((doc) {
      ...
      final matchesSearch = searchQuery.isEmpty || name.contains(searchQuery.toLowerCase()) || id == searchQuery;
      final matchesArea = selectedArea == 'الكل' || location == selectedArea;
      final matchesStatus = selectedStatus == 'الكل' || status == selectedStatus;
            ...
            Row(
              ...
              children: [
                ...
                DropdownButton<String>(
                  ...
                  items: socialStatuses.map((s) => DropdownMenuItem(value: s, child: Text(s))).toList(),
