...
// Function to add a new receipt
          ...
          child: Column(
            ...
            children: [
              DropdownButtonFormField<String>(
                ...
                items: ['الكفالات', 'التبرعات', 'الاشتراكات']
  ...
  Future<void> _printFilteredResults(List<QueryDocumentSnapshot> logs) async {
    try {
      ...
      buffer.writeln(
          '<tr><th>رقم الإيصال</th><th>اسم المصدر</th><th>رقم الهاتف</th><th>المبلغ</th><th>الفئة</th><th>التاريخ</th><th>الملاحظات</th></tr>');

      for (final log in logs) {
        final data = log.data() as Map<String, dynamic>;
        buffer.writeln(
            '<tr><td>${data['receipt_number'] ?? 'غير معروف'}</td>'
            '<td>${data['name'] ?? 'غير معروف'}</td>'
            '<td>${data['phone'] ?? 'غير معروف'}</td>'
            '<td>${data['amount'] ?? 'غير معروف'}</td>'
                ...
                children: [
                  ...
                  Expanded(
                    child: DropdownButtonFormField<String>(
                      ...
                      items: ['الكل', 'الكفالات', 'التبرعات', 'الاشتراكات']
                          .map((category) => DropdownMenuItem(
                                value: category,
                                child: Text(category),
                              ))
