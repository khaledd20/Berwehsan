...
class EditFullCase extends StatefulWidget {
  final String docId;
  const EditFullCase({Key? key, required this.docId}) : super(key: key);
...
class _EditFullCaseState extends State<EditFullCase> {
  ...
  Future<void> _loadCase() async {
    final snapshot = await FirebaseFirestore.instance.collection('cases').doc(widget.docId).get();
    setState(() {
  ...
  Future<void> _saveCase() async {
    if (_formKey.currentState!.validate()) {
      ...
      final newSubIds = selectedSubIds.entries.where((e) => e.value).map((e) => e.key).toList();
      final newChestIds = selectedChestIds.entries.where((e) => e.value).map((e) => e.key).toList();
      caseData['sub_ids'] = newSubIds;
      caseData['chest_ids'] = newChestIds;
      await FirebaseFirestore.instance.collection('cases').doc(widget.docId).update(caseData);
      if (context.mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    final filteredSubs = allSubs.where((sub) =>
      ...
      sub['id'].toString().contains(subSearchQuery)).toList();

    final filteredChests = allChests.where((chest) =>
      chest['name'].toLowerCase().contains(chestSearchQuery.toLowerCase()) ||
      chest['id'].toString().contains(chestSearchQuery)).toList();
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildTextField('عدد أفراد الأسرة', 'family_count', isNumber: true),
                _buildTextField('مقاس الملابس', 'c_size'),
                _buildTextField('مقاس الحذاء', 'S_size'),
                _buildTextField('العمر', 'age', isNumber: true),
                ...
                const Text('بحث في الكفلاء:', style: TextStyle(fontWeight: FontWeight.bold)),
                TextFormField(
                  onChanged: (value) => setState(() => subSearchQuery = value),
                  decoration: const InputDecoration(hintText: 'ابحث بالاسم أو الرقم', border: OutlineInputBorder()),
                ),
                const SizedBox(height: 16),
                const Text('الكفلاء المرتبطون', style: TextStyle(fontWeight: FontWeight.bold)),
                ...filteredSubs.map((sub) {
                  final subId = sub['id'];
                  return CheckboxListTile(
                    title: Text('${sub['name']} (ID: $subId)'),
                    value: selectedSubIds[subId] ?? false,
                    onChanged: (val) => setState(() => selectedSubIds[subId] = val!),
                  );
                }).toList(),
                const SizedBox(height: 24),
                const Text('بحث في الصناديق:', style: TextStyle(fontWeight: FontWeight.bold)),
                TextFormField(
                  onChanged: (value) => setState(() => chestSearchQuery = value),
                  decoration: const InputDecoration(hintText: 'ابحث بالاسم أو الرقم', border: OutlineInputBorder()),
                ),
                const SizedBox(height: 16),
                const Text('الصناديق المرتبطة', style: TextStyle(fontWeight: FontWeight.bold)),
                ...filteredChests.map((chest) {
                  final chestId = chest['id'];
                  return CheckboxListTile(
                    title: Text('${chest['name']} (ID: $chestId)'),
                    value: selectedChestIds[chestId] ?? false,
                    onChanged: (val) => setState(() => selectedChestIds[chestId] = val!),
                  );
                }).toList(),
              ],
            ),
